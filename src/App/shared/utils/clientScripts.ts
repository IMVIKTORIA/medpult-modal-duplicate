...
import { ContractorListData, InsuredListData, RequestListData, TaskListData } from '../types'
...
async function getRequestList(
	...
	const items = Array(7)
		...
		.map((_, index) => {
			...
			const statusValue =
				statusInfo === 'zakryto' ? 'Закрыто' : statusInfo === 'vrabote' ? 'В работе' : 'На ожидании'
			...
			const mockData: RequestListData = {
				...
				topic: new ItemData({ value: 'Согласование медицинских услуг', info: 'test' }),
...
async function getTaskList(page: number, sortData?: SortData): Promise<FetchData<TaskListData>> {
	await randomDelay()
